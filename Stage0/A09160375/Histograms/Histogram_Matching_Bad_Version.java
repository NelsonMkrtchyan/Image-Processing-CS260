...
public class Histogram_Matching_Bad_Version implements PlugInFilter {
...
//      r,g,b chanels arrays
        red[0] = red[0] / (width * height);
        green[0] = green[0] / (width * height);
        blue[0] = blue[0] / (width * height);
//      Normalized r,g,b chanels arrays

        for (int i = 1; i < red.length; i++) {
            red[i] = (red[i] / (width * height)) + red[i - 1];
            green[i] = (green[i] / (width * height)) + green[i - 1];
            blue[i] = (blue[i] / (width * height)) + blue[i - 1];
        }
...
//        double[] newBlue = matchHistograms(blue, BestBlue);
        ...
        for (int row = 0; row < height; row++)
            for (int col = 0; col < width; col++) {
                ...
                for (int j = 0; j < 256; j++) {
                    double diffR = Math.abs(red[color.getRed()] - BestRed[j]);
                    double diffG = Math.abs(green[color.getGreen()] - BestGreen[j]);
                    double diffB = Math.abs(blue[color.getBlue()] - BestBlue[j]);
...
//                ip.putPixelValue(col,row,(newRed[color.getRed()]*(256*256))+ (newGreen[color.getGreen()] * (256)) + (newBlue[color.getBlue()]*1));
